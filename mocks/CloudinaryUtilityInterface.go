...
// UploadCloudinary provides a mock function with given fields: file, filename
func (_m *CloudinaryUtilityInterface) UploadCloudinary(file io.Reader, filename string) (string, error) {
	ret := _m.Called(file, filename)

	if len(ret) == 0 {
		panic("no return value specified for UploadCloudinary")
	}

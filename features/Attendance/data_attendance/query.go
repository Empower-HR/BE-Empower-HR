...
// Get Att
func (am *AttandanceModel) GetAttByPersonalID(personalID uint) ([]attendance.Attandance, error) {
	var attandances []attendance.Attandance

	err := am.db.Where("personal_data_id = ?", personalID).Find(&attandances).Error
	if err != nil {
		return nil, err
	}
...
func (am *AttandanceModel) GetAllAtt() ([]attendance.Attandance, error) {
